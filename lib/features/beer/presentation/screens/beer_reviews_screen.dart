...
class BeerReviewsScreen extends StatelessWidget {
      ...
      body: SafeArea(
        ...
        child: Column(
          children: [
            // Header
            Padding(
              padding: EdgeInsets.symmetric(horizontal: AppSpacings.s16, vertical: AppSpacings.s8),
              child: Row(
                children: [
                  IconButton(
                    icon: const Icon(CupertinoIcons.back, color: AppColors.label),
                    onPressed: () => context.pop(),
                  ),
                  Expanded(
                    child: Column(
                      children: [
                        Text('Recenzje', style: AppTypography.title2),
                        Text(
                          '${beerName.toUpperCase()} · ${histogram.totalCount} OCEN',
                          style: AppTypography.caption.copyWith(color: AppColors.labelSecondary, letterSpacing: 1.2),
                        ),
                      ],
                    ),
                  ),
                ...
                children: [
                  CustomScrollView(
                    slivers: [
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  children: [
                                    _buildHistogramRow('5', histogram.count5, histogram.totalCount),
                                    _buildHistogramRow('4', histogram.count4, histogram.totalCount),
                                    _buildHistogramRow('3', histogram.count3, histogram.totalCount),
                                    _buildHistogramRow('2', histogram.count2, histogram.totalCount),
                                    _buildHistogramRow('1', histogram.count1, histogram.totalCount),
                                  ],
                                ),
                              ),
                            ],
                      ...
                      SliverToBoxAdapter(child: const SizedBox(height: 120)), // Space for FAB
                    ],
                  ),
                  ...
                  Positioned(
                    ...
                    right: 0,
                    child: Container(
                      padding: EdgeInsets.all(AppSpacings.s24).copyWith(bottom: MediaQuery.of(context).padding.bottom + 24),
                      ...
                      child: AppButton(
                        ...
                        onPressed: () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Użyj przycisku na karcie piwa aby ocenić (TODO: Przekazanie Cubit)')));
                        },
                      ),
                    ),
                  ),
  ...
  Widget _buildHistogramRow(String label, int count, int total) {
    ...
    String countText = count > 1000 ? '${(count / 1000).toStringAsFixed(1)}k' : count.toString();
    
    return Padding(
      padding: const EdgeInsets.only(bottom: 8.0),
      child: Row(
        children: [
          Text(label, style: AppTypography.caption.copyWith(fontWeight: FontWeight.bold, color: AppColors.labelSecondary)),
          ...
          SizedBox(
            width: 30,
            child: Text(countText, style: AppTypography.caption.copyWith(color: AppColors.labelSecondary), textAlign: TextAlign.right),
          ),
        ],
      ),
    );
  ...
  Widget _buildReviewCard(Review review) {
    ...
    String timeAgo;
    if (diff.inDays > 0) {
      timeAgo = '${diff.inDays} dni temu';
    } else if (diff.inHours > 0) {
      timeAgo = '${diff.inHours} godz temu';
    } else {
      timeAgo = '${diff.inMinutes} min temu';
    }
          ...
          Row(
            children: [
              CircleAvatar(
                ...
                child: review.userAvatarUrl.isEmpty ? const Icon(CupertinoIcons.person_solid, color: AppColors.labelSecondary) : null,
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(review.userName, style: AppTypography.subhead.copyWith(fontWeight: FontWeight.bold)),
                    SizedBox(height: 4),
                    Row(
                      children: [
                        StarRating(rating: review.overallRating.toDouble(), size: 12),
                        SizedBox(width: 8),
              ...
              Text(timeAgo, style: AppTypography.caption.copyWith(color: AppColors.labelSecondary)),
            ],
          ),
