...
class SupabaseCellarRepository implements ICellarRepository {
  ...
  Future<Either<String, List<Beer>>> getCellar() async {
    try {
      ...
      return left('Nie udało się pobrać piwniczki.');
    }
  }

  @override
  Future<Either<String, void>> addToCellar(String beerId) async {
    try {
      final user = _supabase.auth.currentUser;
      if (user == null) return left('Użytkownik niezalogowany');

      await _supabase.from('cellar').insert({
        'user_id': user.id,
        'beer_id': beerId,
      });
      return right(null);
    } catch (e) {
      return left('Nie udało się dodać do piwniczki.');
    }
  }

  @override
  Future<Either<String, void>> removeFromCellar(String beerId) async {
    try {
      final user = _supabase.auth.currentUser;
      if (user == null) return left('Użytkownik niezalogowany');
      ...
      return left('Nie udało się usunąć z piwniczki.');
    }
  }
}
